...
export async function getAthleteDetail(id: string) {
  ...
  const [athlete, sessionsWindow] = await Promise.all([
    ...
    prisma.session.findMany({
      ...
      include: {
        trainingType: true,
        athleteSessions: { where: { athleteId: id } },
      },
    }),
  ])
  ...
  return {
    ...athlete,
    sessionsWindow: sessionsWindow.map((s) => ({
      id: s.id,
      title: s.title,
      date: s.date,
      ...
      log: s.athleteSessions[0]
        ? {
            id: s.athleteSessions[0].id,
            difficulty: s.athleteSessions[0].difficulty,
            comment: s.athleteSessions[0].comment,
            skipped: s.athleteSessions[0].skipped,
          }
        : null,
    })),
